...
import com.springboot.api.model.DetalhesErro;

public class FuncionarioControllerExceptionHandler {
	@Autowired
	private MessageSource messageSource;
	
	@ExceptionHandler(  FuncionarioNaoEncontradoException.class)
	public ResponseEntity<DetalhesErro> handleEmpresaNaoEncontradaException( FuncionarioNaoEncontradoException ex) {

		DetalhesErro erro = new DetalhesErro();
		erro.setStatus(404);
		erro.setMensagem("Funcionario não pôde ser encontrada!");
		erro.setTimestamp(System.currentTimeMillis());

		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(erro);
	}

	@ExceptionHandler( RemoveFuncionarioException.class)
	public ResponseEntity<DetalhesErro> handleRemoverFuncionarioException( RemoveFuncionarioException ex) {
